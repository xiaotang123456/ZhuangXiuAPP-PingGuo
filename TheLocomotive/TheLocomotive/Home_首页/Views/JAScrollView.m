                    ...
                    withBlock:(ImageViewClick)block;
{
   self = [super initWithFrame:frame];
    if (self) {
        ...
        self.click = block;
        [self loadBaseView];
        NSLog(@"self.dataArry = %ld",self.dataArry.count);
    ...
    for (int i = 0; i<3; i ++) {
        ...
        if (i == 0 && self.dataArry!=nil && self.dataArry.count > 1) {
         //   imageView.image = self.dataArry[self.dataArry.count - 1];//左边
            [imageView sd_setImageWithURL:[NSURL URLWithString:self.dataArry[self.dataArry.count - 1]]];
        }
        if (i == 1 && self.dataArry!=nil && self.dataArry.count > 0) {
         //   imageView.image = self.dataArry[0];//中间
            [imageView sd_setImageWithURL:[NSURL URLWithString:self.dataArry[0]]];
        
        }
        if (i == 2 && self.dataArry !=nil && self.dataArry.count > 1) {
          //  imageView.image = self.dataArry[1];//右边
            [imageView sd_setImageWithURL:[NSURL URLWithString:self.dataArry[1]]];
...
- (void)reloadImage
{
    ...
    [centerImageView sd_setImageWithURL:[NSURL URLWithString:self.dataArry[_currentImageIndex]]];
    //  重新设置左右图片
...
//    rightImageView.image = self.dataArry[rightImageIndex] ;
      [leftImageView sd_setImageWithURL:[NSURL URLWithString:self.dataArry[leftImageIndex]]];
      [rightImageView sd_setImageWithURL:[NSURL URLWithString:self.dataArry[rightImageIndex]]];
