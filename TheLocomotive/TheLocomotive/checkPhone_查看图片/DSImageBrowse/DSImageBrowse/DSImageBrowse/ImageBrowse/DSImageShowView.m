...
@property (nonatomic, weak)   tapblock block;
@end
@implementation DSImageShowView

- (instancetype)initWithItems:(NSArray <DSImageScrollItem *>*)items type:(DSImageShowType)type tapBlock:(void (^)(void))completion;{
    self = [super init];
    ...
    self.numberStr=[NSString stringWithFormat:@"%d/%ld",1,_items.count];
    return self;
}
...
- (NSInteger)currentPage {
    ...
    if (page < 0) page = 0;
    self.numberStr=[NSString stringWithFormat:@"%ld/%ld",page+1,_items.count];
    return page;
}
